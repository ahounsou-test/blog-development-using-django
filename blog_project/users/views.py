...
from .forms import UserRegisterForm, ProfileUpdateFrom, UserUpdateForm
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            #pdb.set_trace()
            form.save()
            Profile(user = User.objects.get(username=request.POST['username']), bio = f" I {request.POST['username']}").save()
